...
public class BossSnake : MonoBehaviour
{
    ...
    {   /* 消失 */
        ...
        if (prediectFlag) flashPosition = skillstate.PreSkillPar04();
        else flashPosition = skillstate.SetSkillPar04();
        Debug.Log(flashPosition);
        /* 顯型 */
        battleState = BattleState.Stand;
        yield return new WaitForSeconds(slaveCd - 2f);
    ...
    IEnumerator Drop()
    {   
        skillstate.SetSkillPar04();     //  出現掉落物
        yield return new WaitForSeconds(0.6f);
        //  射出來
        skillstate.Drop();
