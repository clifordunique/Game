...
public class BigSlim : MonoBehaviour
{
    ...
    [SerializeField]
    private float swordDistance, swordWidth , ImpactDistance;
    [SerializeField]
    ...
    private void Start()
    {
        ...
        GameObject tmep = GameObject.FindGameObjectWithTag("Player");
        player = tmep.GetComponent<Player>();
    ...
    IEnumerator BigSword_Down(bool black_white)
    {   // false is white, true is black
        ...
        for (int i = 1; i < controller.horizontalRayCount; i++)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(rayOrigin, direction, swordDistance, playerLayer);
            Debug.DrawRay(rayOrigin, direction * swordDistance, Color.white);
            if (hit && (black_white == player.colorState))
    ...
    IEnumerator BigSword_Down_Double(bool black_white)
    {   // false is white or true is black
        for (int i = 0; i < controller.horizontalRayCount; i++)
        {
            ...
            RaycastHit2D hitL = Physics2D.Raycast(rayOriginL, Vector2.left, swordDistance, playerLayer);
            ...
            RaycastHit2D hitR = Physics2D.Raycast(rayOriginR, Vector2.right, swordDistance, playerLayer);
            Debug.DrawRay(rayOriginR, Vector2.right * swordDistance, Color.red);
            if ((hitL || hitR) && black_white == player.colorState)
            {   /* hit and give damage to the player */
    ...
    IEnumerator GogoPig()
    {
        ...
        yield return StartCoroutine(ChangeX((5+ PlayerDis/2) * monster.forward, 5, 0.6f));
        yield return StartCoroutine(ChangeY(0, 5, 0.3f));
    ...
    IEnumerator Rolling()
    {
        ...
        yield return new WaitForSeconds(1.0f);
        /* faster and faster */
        yield return StartCoroutine(ChangeX((7 + PlayerDis / 2) * monster.forward, 5, 0.8f));
        if (IsHitTheWall())
   ...
   IEnumerator ChangeX(float speed, float weight, float time)
    ...
    bool IsHitTheWall()
    {
        for (int i = 1; i < controller.horizontalRayCount; i++)
        {
            ...
            Debug.DrawRay(rayOrigin, direction * 0.3f, Color.red);
            if (hit)
            {   /* hit and give damage to the player */
